    ...
    <section ref={ref} className="py-20 bg-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <p className="text-[var(--brand-muted)] mt-4">Real feedback from our valued partners</p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
          {testimonials.map((testimonial, index) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className="bg-white rounded-2xl p-6 shadow-xl border border-[#eef4ff] hover:shadow-2xl transition-all"
            >
              <Quote className="w-10 h-10 text-red-200 mb-4" />
              
              <div className="flex gap-1 mb-4">
                {[...Array(testimonial.rating)].map((_, i) => (
                  <Star key={i} className="w-4 h-4 fill-[var(--brand-red)] text-[var(--brand-red)]" />
                ))}
              </div>

              <p className="text-[var(--brand-copy)] leading-relaxed mb-6 text-sm">
                "{testimonial.quote}"
              </p>

              <div className="flex items-center gap-4 pt-4 border-t border-[#eef4ff]">
                <div className="w-12 h-12 rounded-full bg-linear-to-br from-red-500 to-[var(--brand-red)] flex items-center justify-center text-white font-bold">
                  {testimonial.initials}
                </div>
                <div>
                  <div className="font-bold text-[var(--brand-navy)]">{testimonial.name}</div>
                  <div className="text-sm text-[var(--brand-muted)]">{testimonial.position}</div>
                  <div className="flex items-center gap-1 mt-1">
                    <Building2 className="w-3 h-3 text-[var(--brand-muted)]" />
                    <span className="text-xs text-[var(--brand-muted)]">{testimonial.company}</span>
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        >
          <div className="px-4 py-2 bg-[#eef4ff] rounded-full text-sm text-[var(--brand-blue)]">✓ 100% Satisfaction</div>
          <div className="px-4 py-2 bg-blue-50 rounded-full text-sm text-blue-700">✓ 98% Repeat Business</div>
          <div className="px-4 py-2 bg-[#eef4ff] rounded-full text-sm text-[var(--brand-blue)]">✓ 25+ Years Trust</div>
        </motion.div>
      </div>
    </section>
  );
